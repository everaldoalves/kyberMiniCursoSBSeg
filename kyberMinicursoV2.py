...
class Kyber():
    ...
    def ntt(self, a):
        ...
        if n == 1:
            ...
            even = self.ntt(a[0::2])
            odd = self.ntt(a[1::2])
            w = [self.nth_root_unity ** (i * (self.params.n // n)) for i in range(n)]
    ...
    def inv_ntt(self, a_hat):
        ...
        if n == 1:
            ...
            even = self.inv_ntt(a_hat[0::2])
            odd = self.inv_ntt(a_hat[1::2])
            w = [self.nth_root_unity ** (-i * (self.params.n // n)) for i in range(n)]
            return [(even[i] + w[i] * odd[i]) / 2 for i in range(n // 2)] + [(even[i] - w[i] * odd[i]) / 2 for i in
                                                                             range(n // 2)]
    ...
    def gen_uniformly_random_polynomial_with_xof(self, rho, j, i):
        ...
        while k < self.params.n:
            ...
            b0, b1, b2 = [int.from_bytes(xof.read(1), byteorder='big') for _ in range(3)]
    ...
    def sample_polynomial_with_centered_binomial(self, eta, seed, base_counter):
        ...
        seed_counter_bytes = seed + int(base_counter).to_bytes(1, byteorder='big')
    ...
    def sample_vector_with_centered_binomial(self, eta, seed, base_counter):
        ...
        for i in range(self.params.k):
            ...
            seed_counter_bytes = seed + int(base_counter + i).to_bytes(1, byteorder='big')
            poly_coeffs = self.centered_binomial_distribution(eta, seed_counter_bytes)
    ...
    def vector_sum(self, vector1, vector2):
        ...
        for i in range(self.params.k):
            vector_sum[i] = vector(self.F, vector1[i]) + vector(self.F, vector2[i])
    ...
    def encrypt(self, pk, message, randomness):
        ...
        # converte os objetos encoded_message, self.inv_ntt(self.ntt_vectors_product(ntt_t, ntt_r)) e e2 para vetores do Sage antes de adicioná-los.
        v = vector(self.F, encoded_message) + vector(self.F,
                                                     self.inv_ntt(self.ntt_vectors_product(ntt_t, ntt_r))) + vector(
            self.F, e2)
    ...
    def decrypt(self, sk, ciphertext):
        ...
        ntt_u = self.ntt_vector(u)
        # noisy_message = v - self.inv_ntt(self.ntt_vectors_product(ntt_s, ntt_u))
        # converte o objeto self.inv_ntt(self.ntt_vectors_product(ntt_s, ntt_u)) para um vetor do Sage antes de subtraí-lo
        noisy_message = v - vector(self.F, self.inv_ntt(self.ntt_vectors_product(ntt_s, ntt_u)))
